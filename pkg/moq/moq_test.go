...
func TestMoqPackageAndFolderNamedDifferently(t *testing.T) {
	m, err := New("testpackages/folder_named_differently_from_folder", "")
	if err != nil {
		t.Fatalf("moq.New: %s", err)
	}
	var buf bytes.Buffer
	...
	var shouldNotBeMentioned = []string{
		"github.com/betalo-sweden/moq/pkg/moq/testpackages/folder_named_differently_from_folder",
		"GetFunc: func() *p1.Person",
		"func (mock *PersonStoreMock) Get() *p1.Person",
	}
